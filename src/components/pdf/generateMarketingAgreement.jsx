    ...
    <View style={{
      ...
      {styled && (
        <View style={{ flex: 1, alignItems: 'flex-end' }}>
          <Text style={{ fontSize: 9, color: C.primary, fontWeight: 'bold', textAlign: 'right' }}>הסכם שיווק השקעות</Text>
        </View>
      )}
...
const PageFooter = () => (
  <View fixed style={{
    ...
    <Text style={{ fontSize: 7, color: C.muted }}>INFO@GREENWM.CO.IL</Text>
    <Text style={{ fontSize: 7, color: C.muted }}>03-6456005/6</Text>
    <View style={{ flexDirection: 'row-reverse' }}>
      <Text style={{ fontSize: 7, color: C.muted }}>{'ז׳בוטינסקי 7, '}</Text>
      <Text style={{ fontSize: 7, color: C.muted }}>{'מגדל משה אביב, '}</Text>
      <Text style={{ fontSize: 7, color: C.muted }}>{'רמת גן'}</Text>
    </View>
...
const SignBlock = ({ label, dateValue }) => (
  <View style={{ alignItems: 'center', width: 160 }}>
    ...
    <View style={{ flexDirection: 'row', alignItems: 'flex-end', marginTop: 6, width: 160 }}>
      ...
      <Text style={{ fontSize: 8, marginLeft: 4, textAlign: 'right' }}>:תאריך</Text>
    </View>
  </View>
)
    ...
    <Document>
      ...
      <Page size="A4" style={pageStyle}>
        ...
        <Text style={{ fontSize: 10, textAlign: 'center', marginBottom: 16, color: C.black }}>
          {`שנערך ונחתם ב${d.city || '____________'} ביום ${styled ? fmtDateHebrew() : '__ בחודש___ 2025'}`}
        </Text>
      ...
      <Page size="A4" style={pageStyle}>
        ...
        {/* Option ii */}
        <Para>
          {`ii. כנגד קבלת השירותים, ישלם הלקוח לבעל הרישיון דמי טיפול בשיעור שנתי כולל של %${d.compensationPercent || '____________'} (בתוספת מע"מ) מסך שווי הנכסים המשוערך של הלקוח אשר ביחס אליהם מוענקים שירותים ("נכסי הלקוח") או לחילופין שכר טירחה בגובה ${d.compensationAmount || '____________'} ₪ כפי שהוסכם בין הצדדים.`}
        </Para>
