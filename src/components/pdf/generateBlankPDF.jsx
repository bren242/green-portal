...
const WriteLines = ({ count = 3 }) => (
  <View>
    {Array.from({ length: count }).map((_, i) => (
      <View key={i} style={{ borderBottomWidth: 1, borderBottomColor: C.line, height: 22, marginBottom: 6 }} />
    ))}
  </View>
)

// תוקן: BlankField — רוחב קבוע, לא בתוך flex
const BlankField = ({ width = 80 }) => (
  <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width, height: 16, marginVertical: 2, flexShrink: 0 }} />
)
...
const FinRow = ({ label, isHeader }) => (
  <View style={{
    ...
    <Text style={{
      width: 140, fontSize: isHeader ? 9 : 10,
      fontWeight: isHeader ? 'bold' : 'normal',
      color: C.black, textAlign: 'right',
    }}>{label}</Text>
    {/* עמודת סה"כ */}
    <View style={{ width: 90, paddingHorizontal: 4, justifyContent: 'flex-end', alignItems: 'flex-start' }}>
      {isHeader
        ? <Text style={{ fontSize: 9, fontWeight: 'bold', textAlign: 'right' }}>סה״כ (₪)</Text>
        : <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width: 70, height: 14 }} />
      }
    </View>
    {/* עמודת פירוט */}
    <View style={{ flex: 1, paddingHorizontal: 4, justifyContent: 'flex-end', alignItems: 'flex-start' }}>
      {isHeader
        ? <Text style={{ fontSize: 9, fontWeight: 'bold', textAlign: 'right' }}>פירוט עיקרי</Text>
        : <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width: '90%', height: 14 }} />
      }
    </View>
  </View>
)
  ...
  <Document>
    ...
    <Page size="A4" style={pageStyle}>
      ...
      <View style={{ alignItems: 'center', marginTop: 6, marginBottom: 14 }}>
        ...
        <Text style={{ fontSize: 10, color: C.muted, marginTop: 3 }}>טופס למילוי ידני</Text>
      </View>
      ...
      <View style={{ borderWidth: 1, borderColor: C.primary, borderRadius: 4, padding: 10, marginTop: 14 }}>
        {REG_TEXTS.map((txt, i) => (
          <Text key={i} style={{
            fontSize: 8.5, textAlign: 'right', lineHeight: 1.6,
            color: C.black, marginTop: i > 0 ? 5 : 0,
          }}>{txt}</Text>
        ))}
      </View>
    </Page>
    ...
    <Page size="A4" style={pageStyle}>
      ...
      {/* טבלה פיננסית — ללא רקע, קווים בלבד */}
      ...
      {[
        ...
        <FinRow key={i} label={label} />
      ))}

      <Sep />

      {/* שני תיבות סיכום זה לצד זה — RTL */}
      <View style={{ flexDirection: 'row-reverse', marginBottom: 12 }}>
        <View style={{ flex: 1, borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 8, marginLeft: 6 }}>
          <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 8 }}>סיכום מאזן</Text>
          <Field label="סך נכסים:" width={110} />
          <Field label="סך התחייבויות:" width={110} />
          <Field label="שווי נטו:" width={110} />
        </View>
        <View style={{ flex: 1, borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 8 }}>
          <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 8 }}>מאזן חודשי</Text>
          <Field label="סך הכנסות:" width={110} />
          <Field label="סך הוצאות:" width={110} />
          <Field label="מאזן חודשי:" width={110} />
        </View>
      </View>
    ...
    <Page size="A4" style={pageStyle}>
      ...
      {/* תוקן: wrap={false} + ריבועים ב-row-reverse */}
      ...
      <View wrap={false} style={{ marginBottom: 14 }}>
        <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 6 }}>
          18. מתי תזדקק לכל הכסף שהשקעת? [שאלת חובה]
        </Text>
      ...
      <View wrap={false} style={{ marginBottom: 14 }}>
        <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 6 }}>
          19. לאיזה חלק מתיק ההשקעות תזדקק במהלך 3 השנים הקרובות? [שאלת חובה]
        </Text>
      ...
      <View wrap={false}>
        <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 6 }}>
          23. האם נעזרת בעבר בשירותי מנהל/יועץ השקעות?
        </Text>
      ...
      {/* הסבר סיכון-תשואה */}
      <View style={{ borderWidth: 1, borderColor: C.line, borderRadius: 4, padding: 10, marginTop: 10 }} wrap={false}>
    ...
    {/* ═══════════════════ דף 4: שאלון סיכון + פסקת סירוב + טבלת ניקוד ═══════════════════ */}
    <Page size="A4" style={pageStyle}>
      <SimpleHeader />
      <PageNum />
      ...
      <View style={{ borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 10 }} wrap={false}>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end', marginBottom: 10 }}>
          <Text style={{ fontSize: 10, color: C.black, textAlign: 'right', marginLeft: 8, flexShrink: 0 }}>
            הלקוח סירב להשיב על שאלה/ות מס׳:
          </Text>
          <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, flex: 1, height: 16 }} />
        </View>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end', marginBottom: 10 }}>
          <Text style={{ fontSize: 10, color: C.black, textAlign: 'right', marginLeft: 8, flexShrink: 0 }}>שאלה/ות מס׳</Text>
          <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width: 90, height: 16, marginLeft: 8 }} />
          <Text style={{ fontSize: 10, color: C.black, textAlign: 'right', flexShrink: 0 }}>אינן רלוונטיות עבור הלקוח.</Text>
        </View>
        <Text style={{ fontSize: 9, color: C.muted, textAlign: 'right' }}>
      ...
      {/* כותרת טבלה */}
      <View style={{ flexDirection: 'row-reverse', borderBottomWidth: 1, borderBottomColor: C.black, paddingVertical: 5, paddingHorizontal: 4 }}>
        {['ממוצע ניקוד', 'דרגה', 'שם', 'הפסד מקס׳', 'מניות מקס׳'].map((h, i) => (
          <Text key={i} style={{ flex: 1, fontSize: 9, fontWeight: 'bold', color: C.black, textAlign: 'right' }}>{h}</Text>
        ))}
      </View>
      {[
      ...
      {/* חישוב ידני */}
      <View style={{ flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', marginTop: 16 }} wrap={false}>
        ...
        <Text style={{ fontSize: 14, color: C.black, marginHorizontal: 8 }}>=</Text>
      ...
      <View style={{ flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', marginTop: 10 }}>
        <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, marginLeft: 10 }}>דרגת סיכון מחושבת:</Text>
        <View style={{ width: 36, height: 36, borderWidth: 2, borderColor: C.primary }} />
      </View>
    </Page>
    ...
    <Page size="A4" style={pageStyle}>
      ...
      <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 6 }}>
        24. סיכום וניתוח המידע שהתקבל מהלקוח:
      </Text>
      <WriteLines count={5} />

      <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginTop: 14, marginBottom: 6 }}>
        25. העדפות / הגבלות השקעה לקוח:
      </Text>
      <WriteLines count={3} />

      <Sep />

      <Text style={{ fontSize: 10, fontWeight: 'bold', color: C.primary, textAlign: 'right', marginBottom: 12 }}>
        26. המלצת בעל הרישיון למדיניות השקעה:
      </Text>

      {/* 4 תיבות מדיניות — row-reverse */}
      <View style={{ flexDirection: 'row-reverse', justifyContent: 'space-between', marginBottom: 14 }} wrap={false}>
        {/* אחוז מניות */}
        <View style={{ width: '23%', borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 8, alignItems: 'center' }}>
          <Text style={{ fontSize: 8, color: C.muted, marginBottom: 6, textAlign: 'center' }}>אחוז מניות מקס׳</Text>
          <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 10, color: C.black, marginLeft: 4 }}>%</Text>
            <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width: 40, height: 16 }} />
          </View>
        </View>
        {/* אג"ח קונצרני */}
        <View style={{ width: '23%', borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 8, alignItems: 'center' }}>
          <Text style={{ fontSize: 8, color: C.muted, marginBottom: 6, textAlign: 'center' }}>אג״ח קונצרני</Text>
          <Check label="עד 50%" />
          <Check label="עד 100%" />
        </View>
        {/* מט"ח */}
        <View style={{ width: '23%', borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 8, alignItems: 'center' }}>
          <Text style={{ fontSize: 8, color: C.muted, marginBottom: 6, textAlign: 'center' }}>מט״ח</Text>
          <View style={{ flexDirection: 'row-reverse', justifyContent: 'center' }}>
            <View style={{ marginLeft: 10 }}><Check label="כן" /></View>
            <Check label="לא" />
          </View>
        </View>
        {/* אג"ח דירוג נמוך */}
        <View style={{ width: '23%', borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 8, alignItems: 'center' }}>
          <Text style={{ fontSize: 8, color: C.muted, marginBottom: 6, textAlign: 'center' }}>אג״ח דירוג נמוך</Text>
          <View style={{ flexDirection: 'row-reverse', justifyContent: 'center' }}>
            <View style={{ marginLeft: 10 }}><Check label="כן" /></View>
            <Check label="לא" />
          </View>
      ...
      {/* דרגת סיכון סופית */}
      <View style={{ flexDirection: 'row-reverse', alignItems: 'center', marginBottom: 12 }} wrap={false}>
        <Text style={{ fontSize: 11, fontWeight: 'bold', color: C.primary, marginLeft: 10 }}>דרגת סיכון סופית:</Text>
        <View style={{ width: 36, height: 36, borderWidth: 2, borderColor: C.primary, marginLeft: 10 }} />
        <Text style={{ fontSize: 11, color: C.black, marginLeft: 10 }}>—</Text>
        <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width: 120, height: 16 }} />
      </View>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginVertical: 8 }}>
        {[
          ...
          <View key={n} style={{ alignItems: 'center', marginHorizontal: 10 }}>
            ...
            <Text style={{ fontSize: 7, color: C.muted, marginTop: 3 }}>{label}</Text>
          </View>
        ))}
      </View>
      <Text style={{ fontSize: 8, color: C.muted, textAlign: 'center', marginBottom: 14 }}>* עגל את הדרגה הנבחרת</Text>
    ...
    <Page size="A4" style={pageStyle}>
      ...
      <Text style={{ fontSize: 9, textAlign: 'right', lineHeight: 1.6, marginBottom: 14, color: C.black }}>
      ...
      {/* חתימות לקוח — row-reverse */}
      <View style={{ flexDirection: 'row-reverse', marginBottom: 16 }} wrap={false}>
        <View style={{ flex: 1, paddingLeft: 16 }}>
          <Text style={{ fontSize: 10, color: C.black, textAlign: 'right' }}>חתימת לקוח א׳: X _______________</Text>
          <Text style={{ fontSize: 10, color: C.muted, textAlign: 'right', marginTop: 6 }}>תאריך: ___/___/______</Text>
        </View>
        <View style={{ flex: 1 }}>
      ...
      {/* תיבת סירוב */}
      <View style={{ borderWidth: 1, borderColor: C.black, borderRadius: 4, padding: 10, marginBottom: 14 }} wrap={false}>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end', marginBottom: 10 }}>
          <Text style={{ fontSize: 10, color: C.black, marginLeft: 8, flexShrink: 0 }}>הלקוח סירב להשיב על שאלה/ות מס׳:</Text>
          <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, flex: 1, height: 16 }} />
        </View>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end', marginBottom: 10 }}>
          <Text style={{ fontSize: 10, color: C.black, marginLeft: 8, flexShrink: 0 }}>שאלה/ות מס׳</Text>
          <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, width: 90, height: 16, marginLeft: 8 }} />
          <Text style={{ fontSize: 10, color: C.black, flexShrink: 0 }}>אינן רלוונטיות עבור הלקוח.</Text>
        </View>
        <Text style={{ fontSize: 9, color: C.muted, textAlign: 'right', marginBottom: 8 }}>
          הובהר ללקוח כי אי מסירת המידע עלולה לפגוע באיכות ההמלצה.
        </Text>
        <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end' }}>
          <Text style={{ fontSize: 10, color: C.black, marginLeft: 8, flexShrink: 0 }}>תאריך: ___/___/______</Text>
          <Text style={{ fontSize: 10, color: C.black, marginLeft: 8, flexShrink: 0 }}>חתימה על הסירובים: X _______________</Text>
        </View>
      </View>
      ...
      <View style={{ flexDirection: 'row-reverse', alignItems: 'flex-end' }}>
        <Text style={{ fontSize: 10, color: C.muted, marginLeft: 16, flexShrink: 0 }}>תאריך: ___/___/______</Text>
        <Text style={{ fontSize: 10, color: C.black, marginLeft: 8, flexShrink: 0 }}>חתימת בעל הרישיון:</Text>
        <View style={{ borderBottomWidth: 1, borderBottomColor: C.black, flex: 1, height: 16 }} />
      </View>
    </Page>
